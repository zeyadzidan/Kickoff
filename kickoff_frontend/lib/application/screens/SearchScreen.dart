...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void updateList(String value) {
    ...
    setState(() {
      displayList = LoginScreen.courtsSearch
          .where((element) => element["courtOwnerName"]
              .toLowerCase()
  ...
  Future<void> getListview(String CourtOwnerId) async {
    var response =
        await http.get(Uri.parse('$_url/search/CourtOwner/${CourtOwnerId}'));
    print(response.body);
    ...
    await AnnouncementsHome
        .buildAnnouncements(); // TODO : to get announcements of court
    await ReservationsHome.buildTickets(); // TODO: to get reservations of court
    Navigator.pushNamed(context, '/profileCourt');
  }
            ...
            Expanded(
              child: ListView.builder(
                  ...
                  itemBuilder: (context, index) => Padding(
                        ...
                        child: ListTile(
                          title: Text(
                            "${displayList[index]["courtOwnerName"].toString()}",
                            style: TextStyle(
                              color: Colors.black,
                              fontWeight: FontWeight.bold,
                            ),
                          ...
                          subtitle: Text(
                            "${displayList[index]["distance"].toString()}km",
                            style: TextStyle(
                              color: Colors.black,
                            ),
                          ),
                          trailing: Text(
                            "${displayList[index]["rating"].toString()} \u{2B50}",
                            style: TextStyle(color: Colors.black),
                          ),
